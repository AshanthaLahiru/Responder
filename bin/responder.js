    ...
    async setResponse(context) {
        ...
        if (context.event === 'issues') {
            ...
            if (issuesForRepo && Array.isArray(issuesForRepo)) {
                ...
                let pullRequestsForRepo = (await context.github.pullRequests.getAll(issueParams)).data

                pullRequestsForRepo = pullRequestsForRepo.slice(0, config.response.activities_count || 10)

                if (pullRequestsForRepo && Array.isArray(pullRequestsForRepo)) {
                    let firstResponseTimes = await Promise.all(pullRequestsForRepo.map(async pullRequest => {
                        if (contextSummary.number !== pullRequest.number) {
                            let commentsForRepoParams = context.issue({ number: pullRequest.number })
                            let commentsForIssue = (await context.github.issues.getComments(commentsForRepoParams)).data
                            if (commentsForIssue.length === 0) {
                                return -1
                            } else {
